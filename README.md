...
- Correr `migration.sql` contra Neon (crea `retroalimentacion_ml`, `dispositivos_usuario`,
  `ml_estado_polling`, y quita el `NOT NULL` de `inferencias_ml.humedad`) — **todavía no
...
- `rf_calidad.joblib` y `rf_tiempo_restante.joblib` en `app/ml/artifacts/` siguen siendo
  artefactos viejos e incompatibles (no truenan gracias al manejo defensivo en
  `predictor.py`, pero esas dos predicciones devuelven `null` hasta reentrenar con datos
  reales — ver paso 12 en `ML/README.md` para cuándo ya habrá suficientes).
